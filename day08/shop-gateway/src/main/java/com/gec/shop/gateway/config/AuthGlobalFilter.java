...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    private static final List<String> WHITELIST = List.of("/a/login", "/a/register");
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 兼容旧的 token 参数方式（逐步废弃）
        String oldToken = exchange.getRequest().getQueryParams().getFirst("token");
        if ("123".equals(oldToken)) {
            ServerHttpRequest req = exchange.getRequest().mutate()
                    .header("X-User-Id", "3").header("X-Username", "user1").header("X-Role", "user")
                    .build();
